/// The built-in 8x8 font: 224 glyphs covering bytes 0x20 through 0xFF, taken
/// verbatim from the WASM-4 reference runtime
/// (runtimes/native/src/framebuffer.c, ISC licensed).
///
/// The bits are inverted -- a zero bit is ink. That falls out of how glyphs are
/// drawn: they go through the ordinary 1-bit sprite path, so bit 0 selects
/// DRAW_COLORS field 1 (the text colour) and bit 1 selects field 2 (the
/// background).
let font: [UInt8] = [
